...
if ($pdo instanceof PDO) {
    try {
        ...
        if (is_array($cachedYear)) {
            ...
            $whereSql = 'TRIM(COALESCE(year, "")) = ?';
            $songWhereSql = 'TRIM(COALESCE(s.year, "")) = ?';
            $whereParams = [(string) $year];
            if ($songScope === 'local') {
                $whereSql .= ' AND TRIM(COALESCE(lang, "")) = ?';
                $songWhereSql .= ' AND TRIM(COALESCE(s.lang, "")) = ?';
                $whereParams[] = $localLang;
            }

            $countStmt = $pdo->prepare('SELECT COUNT(*) FROM song WHERE ' . $whereSql);
...
<section class="section">
    <div class="section-head">
        ...
        <div class="music-section-tools">
            <nav class="music-mode-switch music-related-switch music-year-switch" aria-label="<?= music_h(music_label('aria.music_scope_switch', 'Chọn phạm vi bài hát')) ?>">
                <a class="<?= $songScope === 'world' ? 'is-active' : '' ?>" href="<?= music_h(music_url_with_query(music_song_year_url($year), ['scope' => 'world'])) ?>">
                    <i class="fas fa-globe-asia" aria-hidden="true"></i><span><?= music_h(music_label('world', 'Thế giới')) ?></span>
                </a>
                <a class="<?= $songScope === 'local' ? 'is-active' : '' ?>" href="<?= music_h(music_url_with_query(music_song_year_url($year), ['scope' => 'local'])) ?>">
                    <i class="fas fa-map-marker-alt" aria-hidden="true"></i><span><?= music_h(music_label('local', 'Cục bộ')) ?></span>
                </a>
            </nav>
    ...
    <?= music_render_pagination($currentPage, $totalPages, static fn(int $page): string => music_url_with_query(music_song_year_url($year), array_filter(['scope' => $songScope === 'local' ? 'local' : '', 'page_no' => $page]))) ?>
    <div class="grid">
